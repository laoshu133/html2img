...
var processers = {
    ...
    // 裁剪区域
    getCrops: function(options) {
        return this.horseman.evaluate(function(options) {
            return shotTools.getCrops(options);
        }, options);
    },
    ...
    crop: function(cropData, path, options) {
        ...
        var offset = 40;
        var height = cropData.top + cropData.height + offset;

        console.log('xxx', cropData.left);

        return horseman.page.getAsync('viewportSize')
        // 修正截图区域高度
        .then(size => {
            if(size.height < height) {
                tools.log('Processers.crop.resize', height, size.height);

                return horseman.viewport(size.width, height);
            }
        })
        // debug
        .then(() => {
            return horseman.screenshot('__out/page.png');
        })
        .then(() => {
    ...
    makeshot: function(config) {
        ...
        .then(crops => {
            ...
            var imageQuality = config.imageQuality;
    ...
    makelist: function(config) {
        ...
        .then(cropData => {
            ...
            return self.crop(cropData, outCfg.image, {
                quality: imageQuality
            });
        })
        // fit data
